...
namespace AtomEngine
{
	...
	void PixelBuffer::AssociateWithResource(const std::wstring& Name, ID3D12Resource* Resource, D3D12_RESOURCE_STATES CurrentState)
	{
		...
		mWidth = (uint32_t)ResourceDesc.Width;
		...
		const D3D12_RESOURCE_DESC& ResourceDesc, D3D12_CLEAR_VALUE ClearValue, D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr)
	{
...

		{
			...
			ThrowIfFailed(Device->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE,
				&ResourceDesc, D3D12_RESOURCE_STATE_COMMON, &ClearValue, IID_PPV_ARGS(&mResource)));
		}
	...
	void PixelBuffer::ExportToFile(const std::wstring& FilePath)
	{
		...
		CommandContext& Context = CommandContext::Begin(L"Copy mTexture to memory");
		uint32_t RowPitch = Context.ReadbackTexture(TempBuffer, *this);
		Context.Finish(true);

		// バッファメモリへのCPUから見えるポインタを取得します。全範囲を読み取り用にマッピングします。
		void* Memory = TempBuffer.Map();

		// ファイルを開き、ヘッダーに続いてテクセル データを書き込みます。
		...
		// バッファに値が書き込まれていないため、マッピング解除時に null 範囲を使用します。
		TempBuffer.Unmap();
	}
