...
void main(uint3 Gid : SV_GroupID, uint GI : SV_GroupIndex, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    ...
    gCacheW[destIdx + 0] = Linearize(startST | uint2(0, 0));
    gCacheW[destIdx + 8] = Linearize(startST | uint2(8, 0));
    gCacheW[destIdx + 128] = Linearize(startST | uint2(0, 8));
    gCacheW[destIdx + 136] = Linearize(startST | uint2(8, 8));

    GroupMemoryBarrierWithGroupSync();

    uint ldsIndex = (GTid.x << 1) | (GTid.y << 5);

    float w1 = gCacheW[ldsIndex];
